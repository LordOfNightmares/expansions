...
function c54828837.initial_effect(c)
	...
	e2:SetDescription(aux.Stringid(54828837,3))
	e2:SetType(EFFECT_TYPE_IGNITION)
	e2:SetCode(EVENT_FREE_CHAIN)
	e2:SetRange(LOCATION_SZONE)
	e2:SetCountLimit(1)
	e2:SetCondition(c54828837.condition)
	e2:SetTarget(c54828837.target)
	e2:SetOperation(c54828837.activate)
	c:RegisterEffect(e2)
	--negate
...
function c54828837.hfilter(c)
	return c:IsCode(32491822) and not c:IsPublic()
end
function c54828837.setfilter(c)
	return c:IsType(TYPE_SPELL) and not c:IsType(TYPE_FIELD) and c:IsSSetable()
end
function c54828837.setfilter2(c,g)
	return g:IsExists(Card.IsCode,1,c,c:GetCode())
end
function c54828837.condition(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	return Duel.IsExistingMatchingCard(c54828837.hfilter,tp,LOCATION_HAND,0,1,nil)
end
function c54828837.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return end
	if chk==0 then
		local g=Duel.GetMatchingGroup(c54828837.setfilter,tp,LOCATION_DECK+LOCATION_GRAVE,0,nil,e,tp)
		return g:IsExists(c54828837.setfilter2,1,nil,g) and Duel.GetLocationCount(tp,LOCATION_SZONE)>1
	end
end
function c54828837.activate(e,tp,eg,ep,ev,re,r,rp,c)
	--reveal
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_CONFIRM)
	local hg=Duel.SelectMatchingCard(tp,c54828837.hfilter,tp,LOCATION_HAND,0,1,1,nil)
	Duel.ConfirmCards(1-tp,hg)
	Duel.ShuffleHand(tp)
	--selection
	local g=Duel.GetMatchingGroup(c54828837.setfilter,tp,LOCATION_DECK+LOCATION_GRAVE,0,nil,e,tp)
	local dg=g:Filter(c54828837.setfilter2,nil,g)
