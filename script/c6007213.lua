...
function cm.initial_effect(c)
	...
	e1:SetCost(cm.revcost)
	e1:SetTarget(cm.revtg)
	e1:SetOperation(cm.revealop)
	c:RegisterEffect(e1)
	--special summon
...
-- end
function cm.revcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(cm.revtffilter,tp,LOCATION_HAND,0,1,nil) end
...
function cm.revtffilter(c)
	return aux.IsCodeListed(c,6007213) and c:IsType(TYPE_TRAP) and c:IsSSetable()
end
function cm.revtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(cm.revtffilter,tp,LOCATION_HAND,0,1,nil) end
end
function cm.revealop(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	local tc=Duel.SelectMatchingCard(tp,cm.revtffilter,tp,LOCATION_HAND,0,1,1,nil)
	if tc then 
		Duel.SSet(tp,tc)
		--act in set
