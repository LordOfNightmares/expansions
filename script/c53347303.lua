...
function cm.gyspfilter(c)
	return c:IsFaceup() and c:IsSetCard(0xdd) and c:IsAbleToRemoveAsCost()
end
function cm.risecost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 
		and Duel.IsExistingMatchingCard(cm.gyspfilter,tp,LOCATION_MZONE,0,1,e:GetHandler()) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectMatchingCard(tp,cm.gyspfilter,tp,LOCATION_MZONE,0,1,1,nil)
...
function cm.spcon(e,c)
	...
	return Duel.CheckReleaseGroup(c:GetControler(),cm.spfilter,1,nil)
end
function cm.spop(e,tp,eg,ep,ev,re,r,rp,c)
	local g=Duel.SelectReleaseGroup(c:GetControler(),cm.spfilter,1,1,nil)
