...
function c99585850.initial_effect(c)
	...
	es:SetCondition(c99585850.sprcon)
	es:SetOperation(c99585850.sprop)
	c:RegisterEffect(es)
...
function c99585850.sprcon(e,c)
	if c==nil then return true end
	local tp=c:GetControler()
	local g=Duel.GetMatchingGroup(c99585850.sprfilter,tp,LOCATION_MZONE,0,nil)
	return g:IsExists(c99585850.sprfilter1,1,nil,tp,g,c)
end
function c99585850.sprfilter(c)
	return c:IsFaceup() and c:IsAbleToGraveAsCost()
end
function c99585850.sprfilter1(c,tp,g,sc)
	local lv=c:GetLevel()
	return c:IsType(TYPE_XYZ) and g:IsExists(c99585850.sprfilter2,1,c,tp,c)
end
function c99585850.sprfilter2(c,tp,mc)
	local sg=Group.FromCards(c,mc)
	return c:IsCode(97489701) and c:GetLevel()==12
end
function c99585850.sprop(e,tp,eg,ep,ev,re,r,rp,c)
	local g=Duel.GetMatchingGroup(c99585850.sprfilter,tp,LOCATION_MZONE,0,nil)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g1=g:FilterSelect(tp,c99585850.sprfilter1,1,1,nil,tp,g,c)
	local mc=g1:GetFirst()
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g2=g:FilterSelect(tp,c99585850.sprfilter2,1,1,mc,tp,mc)
	g1:Merge(g2)
