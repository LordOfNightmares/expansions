...
function c53184342.initial_effect(c)
	...
	e1:SetCondition(c53184342.spcon)
	...
	c:RegisterEffect(e3)
	Duel.AddCustomActivityCounter(53184342,ACTIVITY_SPSUMMON,c53184342.counterfilter)
end
function c53184342.ndcfilter(c)
	return c:IsFaceup() and c:IsSetCard(0x29)
end
function c53184342.ndcfilter2(c)
	return c:IsFaceup() and not c:IsSetCard(0x29)
end
function c53184342.spcon(e)
	if not Duel.IsExistingMatchingCard(c53184342.ndcfilter2,e:GetHandler():GetControler(),LOCATION_MZONE,0,1,nil) then
		return Duel.IsExistingMatchingCard(c53184342.ndcfilter,e:GetHandler():GetControler(),LOCATION_MZONE,0,1,nil)
	end
end
function c53184342.splimit(e,c,sump,sumtype,sumpos,targetp,se)
...
function c53184342.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	e1:SetTarget(c53184342.splimit)
