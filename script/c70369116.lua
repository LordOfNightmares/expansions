...
function c70369116.initial_effect(c)
	...
	se:SetCode(EFFECT_CANNOT_SPECIAL_SUMMON)
	se:SetProperty(EFFECT_FLAG_PLAYER_TARGET)
	se:SetTargetRange(1,0)
	se:SetReset(RESET_EVENT+RESETS_STANDARD)
	se:SetTarget(c70369116.sumlimit)
	c:RegisterEffect(se)
end
function c70369116.sumlimit(e,c,sump,sumtype,sumpos,targetp,se)
	return not c:IsSetCard(0x10f3) or not c:IsSetCard(0x1046)
end
